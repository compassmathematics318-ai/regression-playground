...
// ===========================================================================
// Linear Train / Test / Export mode switching
// ===========================================================================
...
trainingForm.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch("/api/linear/train", {
      method: "POST",
...
function updateInitializationInputs() {
  ...
  const initialInputs = [initialAInput, initialBInput];
...
resetButton.addEventListener("click", () => {
  ...
  savedManualInitialValues = { a: "0", b: "0" };
  savedRandomSeed = "";
...
function resetTrainingMetrics() {
  ...
  [
    targetAValue, targetBValue,
    currentAValue, currentBValue,
    errorAValue, errorBValue, trainLossValue,
...
function refreshTestAvailability() {
  ...
  testSnapshotHelp.textContent = model.epoch === 0
    ? "Snapshot from before epoch 1. Testing uses these exact starting A and B values."
    : `Snapshot from epoch ${model.epoch.toLocaleString()} / ${state.trainResult.history.length.toLocaleString()}. Testing uses these exact displayed A and B values.`;
...
testForm.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch("/api/linear/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function resetTestSnapshot() {
  testSnapshotHelp.textContent = "Testing uses the exact A and B from the training epoch you were viewing when you entered Test Model.";
  [
    testModelA, testModelB,
    testTargetA, testTargetB,
    testLearnedA, testLearnedB,
    testErrorA, testErrorB,
...
function refreshExportAvailability() {
  ...
  if (!hasTrainingRun) {
    ...
    exportFilename.textContent = "linear_regression_experiment.py";
  ...
  exportTargetSummary.textContent =
    `A=${formatNumber(target.a)}, B=${formatNumber(target.b)}`;

  const initPrefix = initConfig.mode === "random" ? "Random → " : "Manual → ";
  exportInitialSummary.textContent =
    `${initPrefix}A₀=${formatNumber(initial.a)}, B₀=${formatNumber(initial.b)}`;
  ...
  if (state.exportCode) {
    ...
    exportFilename.textContent = state.exportFilename || "linear_regression_experiment.py";
...
async function ensureExportCode() {
  ...
  try {
    const response = await fetch("/api/linear/export/code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
downloadCodeButton.addEventListener("click", () => {
  ...
  anchor.href = url;
  anchor.download = state.exportFilename || "linear_regression_experiment.py";
